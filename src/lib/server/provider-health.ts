...
export const providerHealthIssueMessages: Record<ProviderHealthIssueCode, string> = {
  ...
  MODEL_MISSING_IMAGE_UPSCALE: "图片高清增强配置未配置。",
  MODEL_MISSING_VIDEO_UPSCALE: "视频高清增强配置未配置。",
...
function connectivityUrlFor(provider: ProviderConfig) {
  if (provider.endpointType === "gettoken-banana" || provider.endpointType === "gettoken-veo") return provider.apiUrl;
  if (
