...
export function defaultProviders(): ProviderConfig[] {
  return [
    ...
    },
    {
      ...
      model: env("PROMPT_OPTIMIZER_MODEL", "gpt-5.6-sol"),
      displayName: env("PROMPT_OPTIMIZER_DISPLAY_NAME", "GPT-5.6 Sol"),
...
function mergeStoredProvider(fallback: ProviderConfig, stored: ProviderConfig | undefined) {
  ...
  if (fallback.id === "prompt-optimizer") {
    return {
      ...fallback,
      ...legacyStored,
      apiUrl: legacyStored.apiUrl || fallback.apiUrl,
      model: legacyStored.model || fallback.model,
      displayName: legacyStored.displayName || fallback.displayName,
      models: undefined,
      enabledModels: undefined,
      modelDisplayNames: undefined,
      apiKey: legacyStored.apiKey || fallback.apiKey,
      enabled: hasKey(legacyStored.apiKey || fallback.apiKey),
      endpointType: fallback.endpointType,
      title: legacyStored.title || fallback.title,
      role: fallback.role,
    };
