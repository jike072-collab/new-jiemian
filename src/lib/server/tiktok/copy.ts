...
import { createProviderPromptModelCaller, type PromptModelCaller } from "@/lib/server/prompts";
...
export async function generateMalaysiaTikTokCopy(input: {
  ...
  const caller = input.caller || createProviderPromptModelCaller(undefined, TIKTOK_COPY_MODEL);
  const callInput = {
    systemPrompt,
