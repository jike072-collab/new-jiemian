...
export function malaysiaCommerceHookPromptLibrary(directions?: readonly CanvasCommerceDirection[]) {
  ...
  return JSON.stringify({
    ...
    strongHookExamples: [
      "痛点反转式：Hampir nak return, tapi detail warna kasut ni terus ubah fikiran. 同一句完成负面预期和可见细节反转。",
      "避坑警示式：Korang yang selalu salah pilih warna kasut, tengok detail sisi ni dulu. 必须明确说出要避开的选择问题和马上展示的细节。",
      "情绪发现式：Akhirnya jumpa kasut yang buat detail outfit tak tenggelam lagi. 2-7 秒必须展示同一套穿搭如何被鞋子细节改变。",
      "信息差式：Kalau tak nampak detail sisi kasut ni, memang susah nak tahu bezanya. 首帧露出线索，随后展示完整细节。",
      "数字兑现式：Dua detail kasut ni terus ubah keseluruhan look bila bergerak. 后续镜头必须分别展示两处不同且可见的细节。",
      "细节发现式：Baru perasan detail warna pada sisi kasut ni bila kena cahaya. 只能发现图片能证明的颜色、轮廓或结构。",
    ],
    globalRules: [
      ...
      "每条首句必须是 8-18 个马来语词，并同时包含明确的信息缺口、冲突、警示、发现、反转或可兑现承诺，以及至少一个图片可见的鞋子对象或细节；3-7 词的泛问句、单纯惊叹和空喊停留一律不作为最终钩子。",
