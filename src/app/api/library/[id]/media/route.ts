...
import { LibraryOperationError, resolveLibraryMediaForOwner } from "@/lib/server/library";
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const output = await resolveLibraryMediaForOwner(id, session.user.local_user_id);
