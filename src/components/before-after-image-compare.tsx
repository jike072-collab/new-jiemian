...
export function BeforeAfterImageCompare({
  ...
  return (
    ...
    >
      {mediaType === "video" ? (
        <>
          <video className="compare-before" src={beforeSrc} poster={beforePoster} aria-label={beforeAlt} autoPlay muted loop playsInline preload="metadata" />
          <video className="compare-after" src={afterSrc} poster={afterPoster} aria-label={afterAlt} autoPlay muted loop playsInline preload="metadata" />
        </>
      ) : (
        <>
