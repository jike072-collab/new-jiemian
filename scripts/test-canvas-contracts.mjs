...
assert.match(imageSubmission, /items\.forEach\(\(item, index\) => addResultNode\(generatorId, item, null, requestOffsets\[requestIndex\] \+ index, count\)\)/);
...
assert.match(workspace, /panOnDrag=\{presentation === "vozeb" && !compactViewport \? \[1\] : true\}/);
assert.match(workspace, /selectionOnDrag=\{presentation === "vozeb" && !compactViewport\}/);
assert.match(workspace, /selectionKeyCode=\{presentation === "vozeb" \? null : "Shift"\}/);
...
assert.match(shortcutsPanel, /框选多个节点.*拖动空白处/);
assert.match(shortcutsPanel, /Space.*拖动/);
assert.doesNotMatch(shortcutsPanel, /按住 Shift，再从画布空白处拖动/);
