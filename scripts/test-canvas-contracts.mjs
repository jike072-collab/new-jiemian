...
const mediaSizing = read("src/lib/canvas/media-sizing.ts");
const canvasVideoPreview = read("src/components/canvas/canvas-video-preview.tsx");
...
assert.match(libraryRoute, /includeFailed: true/);
assert.match(libraryRoute, /recoveryItems/);
...
assert.match(workspace, /CanvasVideoPreview src=\{item\.output\.url\}/);
assert.match(canvasNode, /CanvasVideoPreview/);
assert.match(canvasVideoPreview, /IntersectionObserver/);
assert.match(canvasVideoPreview, /rootMargin: "240px"/);
assert.match(canvasVideoPreview, /setActive\(true\)/);
...
assert.match(workspace, /function canvasConnectionError/);
assert.match(workspace, /当前视频模型不支持参考视频/);
assert.match(workspace, /当前视频模型不支持参考音频/);
assert.match(workspace, /当前视频模型最多支持/);
