...
assert.match(tikTokCopy, /const TIKTOK_COPY_MODEL = "gpt-5\.6-sol"/);
assert.match(tikTokCopy, /createProviderPromptModelCaller\(undefined, TIKTOK_COPY_MODEL\)/);
assert.doesNotMatch(tikTokCopy, /createNewApi(?:Admin)?PromptModelCaller/);
assert.doesNotMatch(tikTokCopy, /gpt-5\.6-luna/);
...
assert.match(canvasNode, /className="nodrag nowheel nopan"/);
assert.match(canvasNode, /onPointerDown=\{\(event\) => event\.stopPropagation\(\)\}/);
assert.match(canvasNode, /data\.mediaOrigin === "upload" \? null : <StatusLine/);
assert.match(workspace, /item\.mode === "canvas-upload" \? null : <small>/);
...
assert.match(workspace, /hydrateMediaNodes\(nodesRef\.current, items, true\)/);
assert.match(workspace, /reconcileTerminalVideoGenerators/);
assert.match(workspace, /releasedGeneratorCount/);
...
assert.match(workspace, /canvasLibraryMediaUrl\(libraryItemId, item\.updatedAt\)/);
...
assert.match(workspace, /viewportInteractingRef/);
assert.match(workspace, /window\.requestAnimationFrame\(flushDeferredCanvasSync\)/);
assert.match(workspace, /uploadCanvasMediaFiles\(result\.files, position, result\.truncated\)/);
assert.match(canvasMediaRoute, /createHash\("sha256"\)/);
assert.match(canvasMediaRoute, /contentSha256: fingerprint/);
assert.match(canvasMediaRoute, /item\.params\.contentSha256 === fingerprint/);
assert.match(canvasMediaRoute, /const uploadScopeId = workspace\?\.ownerId \|\| session\.user\.local_user_id/);
assert.match(canvasMediaRoute, /const key = `\$\{canvasScope\}:\$\{uploadScopeId\}:\$\{type\}:\$\{fingerprint\}`/);
assert.match(canvasMediaRoute, /if \(existing\) return NextResponse\.json\(\{ ok: true, item: scopedCanvasItem\(existing, canvasScope\), reused: true \}\)/);
...
assert.match(videoTrimmer, /MAX_CLIP_SECONDS = 14\.9/);
...
assert.match(videoTrimRoute, /libraryItemIds/);
assert.match(videoTrimRoute, /MAX_BATCH_VIDEO_TRIMS = 12/);
assert.match(videoTrimRoute, /probeVideoDuration/);
assert.match(videoTrimRoute, /for \(const id of libraryItemIds\)/);
assert.match(videoTrimRoute, /mode: "video-trim"/);
assert.match(videoTrimRoute, /sourceLibraryItemId/);
assert.match(videoTrimService, /MAX_VIDEO_TRIM_SECONDS = 14\.9/);
