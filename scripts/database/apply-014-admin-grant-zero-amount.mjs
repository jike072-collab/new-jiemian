...
const version = "014_admin_grant_zero_amount";
...
const pool = new Pool({
  ...
  query_timeout: 15000,
  application_name: "aohuang_apply_admin_grant_zero_amount_014",
});
...
try {
  ...
  if (existing.rows[0]) {
    if (existing.rows[0].checksum !== checksum) throw new Error("Migration 014 checksum mismatch.");
