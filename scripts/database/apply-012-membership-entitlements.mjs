...
const version = "012_expand_membership_entitlements";
...
const pool = new Pool({
  ...
  query_timeout: 15000,
  application_name: "aohuang_apply_membership_entitlements_012",
});
...
try {
  ...
  if (existing.rows[0]) {
    if (existing.rows[0].checksum !== checksum) throw new Error("Migration 012 checksum mismatch.");
