...
assert(
  upscaleImageRoute.includes("runUpscaleImage(file, scale, session.user.local_user_id, taskId, idempotencyKey, estimatedQuotaUnits)"),
  "image upscale route must pass current owner and billing context",
);
assert(
  upscaleVideoRoute.includes("runSubmitVideoUpscale(file, scale, session.user.local_user_id, taskId, idempotencyKey, estimatedQuotaUnits)"),
  "video upscale route must pass current owner and billing context",
);
