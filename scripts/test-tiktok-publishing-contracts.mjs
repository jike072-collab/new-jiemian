...
assert.match(publishRoute, /zernioAccountId: String\(body\.zernioAccountId \|\| ""\)/);
...
assert.match(publisher, /transientReadRetryDelays = \[600, 1_800\]/);
assert.match(publisher, /transientReadStatuses\.has\(error\.status\)/);
assert.match(publisher, /void loadJobs\(\)\.catch\(\(\) => undefined\)/);
assert.match(publisher, /setConnectionError\(apiMessage/);
assert.match(publisher, /setCreatorError\(apiMessage/);
assert.match(publisher, /重新读取发布权限/);
assert.doesNotMatch(publisher, /Promise\.all\(\[fetchJson<ConnectionResponse>/);
...
assert.match(copyService, /createProviderPromptModelCaller\(undefined, TIKTOK_COPY_MODEL\)/);
assert.doesNotMatch(copyService, /createNewApi(?:Admin)?PromptModelCaller/);
assert.doesNotMatch(copyService, /gpt-5\.6-luna/);
...
assert.match(copyService, /避免空泛的 ‘terus nampak lain’/);
