...
`ZERNIO_EXTRA_CREDENTIALS_JSON`, a JSON array of objects with `id`, `apiKey`,
`profileIds`, and an optional canvas `displayName`; keys stay in the production
environment and bindings store only the credential ID. Site members can bind multiple accounts and select the target
