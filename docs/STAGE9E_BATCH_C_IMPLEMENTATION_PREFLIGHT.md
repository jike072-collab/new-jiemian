...
This document does not enable dual-write, does not switch any read path, does not change feature flags, does not execute rollback, does not write staging or production business data, does not call NewAPI or a real provider, does not trigger generation, and does not incur cost.
...
Do not skip the planning-only steps before any enablement planning.
...
- staging DB read: yes, only for later evidence review
- staging DB write: no
...
- parity expectations are unclear
