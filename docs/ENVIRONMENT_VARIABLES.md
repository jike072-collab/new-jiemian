...
| `REMOTE_MEDIA_ALLOWED_HOSTS` | Production allowlist for remote media downloads and redirects. | No | Optional | Required for production remote downloads | Empty | Comma-separated exact hosts or `*.subdomain.example` rules; placeholders, `*`, localhost, and private-network targets are not allowed | remote media download |
...
| `IMG2_IMAGE_API_URL`, `IMG2_IMAGE_MODEL`, `IMG2_IMAGE_DISPLAY_NAME`, `IMG2_IMAGE_API_KEY`, `IMG2_IMAGE_ENDPOINT_TYPE` | Secondary image provider. `nianhuaapi.com` is retired and is forced off even if configured. | Key is sensitive | Optional | Optional unless enabled | No built-in URL; configure the active image provider explicitly | HTTP(S), supported endpoint type | providers |
...
| `GETTOKEN_VEO_API_URL`, `GETTOKEN_VEO_API_KEY` | GetToken Veo 3.1 Pro/Fast video provider; falls back to the GetToken Banana URL/key when unset. | Key is sensitive | Optional | Optional unless enabled | GetToken OpenAPI URL, Banana credentials fallback | HTTP(S) | providers |
...
- Use exact hosts or explicit subdomain rules only, for example `media.allowed.example,*.cdn.allowed.example`.
