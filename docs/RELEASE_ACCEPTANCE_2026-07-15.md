...
- Result actions use a neutral default state and only the hovered or keyboard-focused action receives the accent color.
- Library tiles and detail overlays show the normalized generating model name.
- Dismissing the generation progress toast only hides the toast; the running task and pending result slots remain active.
...
| Result action and model labels | Pass | No result action has a fixed accent class; hover changes the active action, and library tile/detail fixtures show the model name |
