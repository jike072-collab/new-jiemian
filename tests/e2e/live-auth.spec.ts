...
test("account entitlements and library actions remain visible without mutating production data", async ({ page }, testInfo: TestInfo) => {
  ...
  await expect(promptDialog.getByRole("button", { name: "创作目的" })).toContainText("自动判断");
  await expect(promptDialog.getByLabel("编辑任务")).toHaveCount(0);
  await expect(promptDialog.getByLabel("视频类型")).toHaveCount(0);
